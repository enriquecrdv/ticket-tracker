...
memanto remember "Generalized principle or rule" --type TYPE --tags "tag1,tag2" --confidence <0.0-1.0> --provenance PROVENANCE --source codex
...
memanto recall "query string" --limit 10 --type TYPE --min-similarity 0.8 --tool codex

# Temporal search variants (no query needed)
memanto recall --recent --limit 10 --tool codex            # newest memories first
memanto recall --as-of "YYYY-MM-DD" --tool codex           # memory state at a past point in time
memanto recall --changed-since "last 7 days" --tool codex  # memories created or updated recently

# Grounded RAG answer (Synthesizes memory into a direct answer)
memanto answer "Question about past decisions or commitments" --tool codex
...
4. **Missing Metadata Flags**
   ...
   - On `recall` and `answer`, always pass `--tool codex`. Reads carry no `--source`, and this is how Memanto identifies the calling agent.
...
memanto recall "authentication setup guidelines" --limit 10 --tool codex
```

### Workflow 2: Learning from Error / Correction
```bash
# Store lesson immediately after fixing a tricky bug or receiving user correction
memanto remember "Next.js standalone build requires output: 'standalone' in next.config.js" --type learning --tags "nextjs,build,deployment" --confidence 1.0 --provenance corrected --source codex
```

### Workflow 3: User Sets Architectural Constraint
```bash
# Store user-defined rule immediately
memanto remember "Use UUID v4 for all primary keys across all PostgreSQL tables" --type instruction --tags "database,postgresql,schema" --confidence 1.0 --provenance explicit_statement --source codex
```
