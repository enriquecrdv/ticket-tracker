...
export async function PATCH(
  ...
  const parsed = z.object({ status: z.enum(["pendiente", "seguimiento", "espera_cliente", "cerrado"]).optional(), customerNumber: z.string().regex(/^\d{9}$/).optional(), customerName: z.string().trim().min(2).max(150).optional() }).refine((data) => data.status || data.customerNumber, "Sin cambios").safeParse(await request.json());
    ...
    data: {
      ...
      history: {
        create: {
          ...
          detail: parsed.data.customerNumber ? `Número de cliente asignado: ${parsed.data.customerNumber}.` : `Estado actualizado a ${parsed.data.status}.`,
