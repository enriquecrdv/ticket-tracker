...
const priorityMap = { baja: "BAJO", media: "MEDIO", alta: "ALTO" } as const;
...
export async function PATCH(
  ...
  const ticket = await prisma.ticket.update({
    ...
    data: {
      ...
      ...(parsed.data.priority ? { priority: priorityMap[parsed.data.priority] } : {}),
      ...
      history: {
        create: {
          ...
          detail: parsed.data.customerNumber ? `Número de cliente asignado: ${parsed.data.customerNumber}.` : parsed.data.assignedToId !== undefined ? "Folio reasignado por administración." : parsed.data.priority ? `Prioridad actualizada a ${parsed.data.priority}.` : parsed.data.adminAlert !== undefined ? `Alerta administrativa ${parsed.data.adminAlert ? "activada" : "desactivada"}.` : `Estado actualizado a ${parsed.data.status}.`,
